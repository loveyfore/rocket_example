...
use rocket_example_core::post_service::PostService;
use serde_json::json;

use migration::{MigratorTrait, sea_orm};
...
async fn create(conn: Connection<'_, Db>, post_form: Form<post::Model>) -> Flash<Redirect> {
    ...
    PostService::create_post(db, form)
        .await
...
async fn update(
    ...
    PostService::update_post_by_id(db, id, form)
        .await
...
async fn list(
    ...
    let (posts, num_pages) = PostService::find_posts_in_page(db, page, posts_per_page)
        .await
...
async fn edit(conn: Connection<'_, Db>, id: i32) -> Template {
    ...
    let post: Option<post::Model> = PostService::find_post_by_id(db, id)
        .await
...
async fn destroy(conn: Connection<'_, Db>) -> Result<(), rocket::response::Debug<String>> {
    ...
    PostService::delete_all_posts(db)
        .await
...
async fn run_migrations(rocket: Rocket<Build>) -> fairing::Result {
    if let Some(conn) = rocket.state::<DatabaseConnection>() {
